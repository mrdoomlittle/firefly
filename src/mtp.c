...
ffly_mtp_sndhdr(FF_SOCKET *__sock, ffly_mtp_hdrp __hdr) {
	ff_err_t err;
	ff_uint_t l = ffly_str_len(__hdr->file)+1;
	ff_net_send(__sock, &l, sizeof(ff_uint_t), &err);
	if (_err(err))
		_ret;
...
ffly_mtp_rcvhdr(FF_SOCKET *__sock, ffly_mtp_hdrp __hdr) {
	ff_err_t err;
	ff_uint_t l;
	ff_net_recv(__sock, &l, sizeof(ff_uint_t), &err);
	if (_err(err))
		_ret;
...
ffly_mtp_rcv(FF_SOCKET *__sock, ff_uint_t *__size, ff_err_t *__err) {
	*__err = FFLY_SUCCESS;
	ff_net_recv(__sock, __size, sizeof(ff_uint_t), __err);
	if (_err(*__err))
		return NULL;
