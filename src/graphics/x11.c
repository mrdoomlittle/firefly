...
ff_err_t ffly_x11_query_pointer(struct ffly_x11_ctx *__ctx, ff_i16_t *__root_xa, ff_i16_t *__root_ya, ff_i16_t *__wd_xa, ff_i16_t *__wd_ya) {
    Window root, child; // not needed for now
    int root_xa, root_ya, wd_xa, wd_ya;
    int unsigned mask;
    XQueryPointer(__ctx->d, XRootWindow(__ctx->d, 0), &root, &child, &root_xa, &root_ya, &wd_xa, &wd_ya, &mask);    
