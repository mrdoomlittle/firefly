...
mdl::firefly::types::err_t mdl::firefly::graphics::gpu_draw_pixmap(types::gpu_dr_pm_mem_t& __gpu_dr_pm_mem, uint_t __xfs, uint_t __yfs, types::pixmap_t __pixbuff, uint_t __pb_xlen, uint_t __pb_ylen, uint_t __pm_xfs, types::pixmap_t __pixmap, uint_t __pm_xlen, uint_t __pm_ylen, uint_t __rpm_xlen, u16_t __angle) {
	...
	if (!__gpu_dr_pm_mem.inited) {
		if ((any_err = cudaMalloc((void **)&__gpu_dr_pm_mem.xfs, sizeof(uint_t))) != cudaSuccess) {
			fprintf(stderr, "draw_pixmap: cuda, failed to alloc memory for 'xfs', cu_errno: %d\n", any_err);
			ffly_errno = FF_ERR_CFTAM;
			return FFLY_FAILURE;
		}

		if ((any_err = cudaMalloc((void **)&__gpu_dr_pm_mem.yfs, sizeof(uint_t))) != cudaSuccess) {
			fprintf(stderr, "draw_pixmap: cuda, failed to alloc memory for 'yfs', cu_errno: %d\n", any_err);
			ffly_errno = FF_ERR_CFTAM;
			return FFLY_FAILURE;
		}

		if ((any_err = cudaMalloc((void **)&__gpu_dr_pm_mem.pb_xlen, sizeof(uint_t))) != cudaSuccess) {
			fprintf(stderr, "draw_pixmap: cuda, failed to alloc memory for 'pb_xlen', cu_errno: %d\n", any_err);
			ffly_errno = FF_ERR_CFTAM;
			return FFLY_FAILURE;
		}

		if ((any_err = cudaMalloc((void **)&__gpu_dr_pm_mem.pm_xlen, sizeof(uint_t))) != cudaSuccess) {
			fprintf(stderr, "draw_pixmap: cuda, failed to alloc memory for 'pm_xlen', cu_errno: %d\n", any_err);
			ffly_errno = FF_ERR_CFTAM;
			return FFLY_FAILURE;
		}

		if ((any_err = cudaMalloc((void **)&__gpu_dr_pm_mem.pm_xfs, sizeof(uint_t))) != cudaSuccess) {
			fprintf(stderr, "draw_pixmap: cuda, failed to alloc memory for 'pm_xfs', cu_errno: %d\n", any_err);
			ffly_errno = FF_ERR_CFTAM;
			return FFLY_FAILURE;
		}

		if ((any_err = cudaMalloc((void **)&__gpu_dr_pm_mem.angle, sizeof(u16_t))) != cudaSuccess) {
			fprintf(stderr, "draw_pixmap: cuda, failed to alloc memory for 'angle', cu_errno: %d\n", any_err);
			ffly_errno = FF_ERR_CFTAM;
			return FFLY_FAILURE;
		}
		...
		if (__xfs == 0) {
			if ((any_err = cudaMemcpy(__gpu_dr_pm_mem.xfs, &__xfs, sizeof(uint_t), cudaMemcpyHostToDevice)) != cudaSuccess) {
				fprintf(stderr, "draw_pixmap: cuda, failed to copy memory to device, cu_errno: %d\n", any_err);
	...
	if (__gpu_dr_pm_mem._pb_size != pb_size)
	{
		...
		if ((any_err = cudaMalloc((void **)&__gpu_dr_pm_mem.pixbuff, pb_size * sizeof(types::__pixmap_t))) != cudaSuccess) {
			fprintf(stderr, "cuda: failed to call Malloc, cu_errno: %d\n", any_err);
			ffly_errno = FF_ERR_CFTAM;
			return FFLY_FAILURE;
		}
	...
	if (__gpu_dr_pm_mem._xfs != __xfs) {
		if ((any_err = cudaMemcpy(__gpu_dr_pm_mem.xfs, &__xfs, sizeof(uint_t), cudaMemcpyHostToDevice)) != cudaSuccess) {
			fprintf(stderr, "cuda: failed to call Memcpy, cu_errno: %d\n", any_err);
			ffly_errno = FF_ERR_CFTCM;
			return FFLY_FAILURE;
		}
	...
	if ((any_err = cudaDeviceSynchronize()) != cudaSuccess) {
		fprintf(stderr, "cuda: failed to sync device, cu_errno: %d\n", any_err);
