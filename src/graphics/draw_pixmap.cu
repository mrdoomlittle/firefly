...
__global__ void cu_draw_pixmap(mdl::uint_t *__xoffset, mdl::uint_t *__yoffset, boost::uint8_t *__pixbuff, mdl::uint_t *__pb_xlen, boost::uint8_t *__pixmap, boost::uint16_t *__angle) {
	//mdl::uint_t pixbuff_pos = ((threadIdx.x + (*__xoffset)) + ((blockIdx.x + (*__yoffset)) * (*__pb_xlen))) * 4;
	mdl::uint_t pixbuff_pos = ((threadIdx.x + (*__xoffset)) + (blockIdx.x * (*__pb_xlen))) * 4;
...
boost::int8_t mdl::firefly::graphics::draw_pixmap(uint_t __xoffset, uint_t __yoffset, boost::uint8_t *__pixbuff, uint_t __pb_xlen, uint_t __pb_ylen, boost::uint8_t *__pixmap, uint_t __pm_xlen, uint_t __pm_ylen, boost::uint16_t __angle) {
	...
	uint_t pixbuff_size = (__pm_ylen * __pb_xlen) * 4;//(__pb_xlen * __pb_ylen) * 4;
	...
	uint_t pb_offset = __yoffset >= (__pb_ylen - __pm_ylen)? ((__pb_ylen - __pm_ylen) * __pb_xlen) * 4: (__yoffset * __pb_xlen) * 4;

	if ((any_error = cudaMemcpy(pixbuff, __pixbuff + pb_offset, pixbuff_size * sizeof(boost::uint8_t), cudaMemcpyHostToDevice)) != cudaSuccess) {
		fprintf(stderr, "cuda: failed to call Memcpy, errno: %d\n", any_error);
		return FFLY_FAILURE;
	}
	...
	cu_draw_pixmap<<<__pm_ylen, __pm_xlen>>>(xoffset, yoffset, pixbuff, pb_xlen, pixmap, angle);

	if ((any_error = cudaMemcpy(__pixbuff + pb_offset, pixbuff, pixbuff_size * sizeof(boost::uint8_t), cudaMemcpyDeviceToHost)) != cudaSuccess) {
		fprintf(stderr, "cuda: failed to call Memcpy, errno: %d\n", any_error);
		return FFLY_FAILURE;
	}
