...
mdl::firefly::types::err_t mdl::firefly::graphics::x11_window::begin(u16_t __wd_xaxis_len, u16_t __wd_yaxis_len, char const *__frame_title) {
	...
	do {
		...
		for (std::size_t o = 0; o != screen_count; o ++) {
			...
			if (XQueryPointer(_display, r_windows[o], &dmmy_root, &dmmy_child, &this->pointer_coords->root.xaxis, &this->pointer_coords->root.yaxis, &dmmy_xaxis, &dmmy_yaxis, &m_mask)) break;
		}

		if ((this->pointer_coords->root.xaxis > this->wd_coords->xaxis && this->pointer_coords->root.xaxis < (this->wd_coords->xaxis + __wd_xaxis_len))
			&& (this->pointer_coords->root.yaxis > this->wd_coords->yaxis && this->pointer_coords->root.yaxis < (this->wd_coords->yaxis + __wd_yaxis_len))) {
			this->pointer_coords->wd.xaxis = (this->pointer_coords->root.xaxis-this->wd_coords->xaxis);
			this->pointer_coords->wd.yaxis = (this->pointer_coords->root.yaxis-this->wd_coords->yaxis);
		...
		while (XPending(_display) > 0) {
			...
			switch(_xevent.type) {
				...
				case KeyPress:
					event.first.event_id = system::event_id::key;
					this->key_press = true;
				...
				case ButtonPress:
					event.first.event_id = system::event_id::btn;
...
//		printf("mc_rx: %d, mc_ry: %d\n", this->pointer_coords->root.xaxis, this->pointer_coords->root.yaxis);
//		printf("mc_wdx: %d, mc_wdy: %d\n", this->pointer_coords->wd.xaxis, this->pointer_coords->wd.yaxis);
