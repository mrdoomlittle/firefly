...
//	ffly_printf("next op.\n");
	...
	__resin->ip_off = 0;
	ff_u8_t opno;	
	if ((opno = get_8l(__resin, &err)) >= NOOP) {
	...
	__asm__("_arm:\n\t");
	{
		...
		stack_get(__resin, (ff_u8_t*)&lt, l, la);
		stack_get(__resin, (ff_u8_t*)&rt, l, ra);
		ff_u64_t dst;
	...
	__asm__("_exit:\n\t");
	{
		...
		ff_err_t exit_code;
		if (__exit_code != NULL) {
			stack_get(__resin, (ff_u8_t*)&exit_code, sizeof(ff_err_t), addr);
