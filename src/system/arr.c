...
ffly_err_t ffly_arr_resize(struct ffly_arr *__arr, mdl_uint_t __nblk_c) {
	if ((__arr->p = __ffly_mem_realloc(__arr->p, (__arr->blk_c = __nblk_c)*__arr->blk_size)) == NULL) {
...
ffly_err_t ffly_arr_de_init(struct ffly_arr *__arr) {
	if (__arr->p != NULL) __ffly_mem_free(__arr->p);
	return FFLY_SUCCESS;
}
