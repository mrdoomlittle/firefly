...
mdl_u64_t static
mdl_hash(mdl_u8_t const *__key, mdl_uint_t __bc) {
...
mdl_u64_t static
mdl_dvhash(mdl_u8_t const *__key, mdl_uint_t __bc) {
	mdl_u8_t const *p = __key;
	mdl_u8_t const *end = p+__bc;
	mdl_u64_t ret_val = 1;
	while(p != end) {
...
mdl_u64_t static
mdl_ahash(mdl_u8_t const *__key, mdl_uint_t __bc) {
	mdl_u8_t const *p = __key;
	mdl_u8_t const *end = p+__bc;
	mdl_u64_t ret_val = 1;
	while(p != end) {
...
mdl_u64_t(*ffly_hash)(mdl_u8_t const*, mdl_uint_t) = &mdl_ahash;
