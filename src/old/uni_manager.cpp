...
mdl::firefly::types::err_t mdl::firefly::uni_manager::init(uint_t __xa_split, uint_t __ya_split, uint_t __za_split) {
	uint_t cnk_xa_len = this->xa_len/__xa_split;
	if (cnk_xa_len%1) {
		system::io::printf(stderr, "can't use %d for the chunk xaxis split.\n", __xa_split);
		return FFLY_FAILURE;
	}

	uint_t cnk_ya_len = this->ya_len/__ya_split;
	if (cnk_ya_len%1) {
		system::io::printf(stderr, "can't use %d for the chunk yaxis split.\n", __ya_split);
		return FFLY_FAILURE;
	}

	uint_t cnk_za_len = this->za_len/__za_split;
	if (cnk_za_len%1) {
		system::io::printf(stderr, "can't use %d for the chunk zaxis split.\n", __za_split);
		return FFLY_FAILURE;
	}
	...
	if ((this->cnk_id_indx = (uint_t**)memory::mem_alloc(cnk_c*sizeof(uint_t*))) == NULL) {
		system::io::printf(stderr, "uni_manager: failed to alloc memory for 'chunk_id_index', errno: %d\n", errno);
		return FFLY_FAILURE;
	}

	if ((this->_1d_uni_pm = (types::_1d_pm_t*)memory::mem_alloc(cnk_c*sizeof(types::_1d_pm_t*))) == NULL) {
		system::io::printf(stderr, "uni_manager: failed to alloc memory for '_1d_uni_pm', errno: %d\n", errno);
		memory::mem_free(this->cnk_id_indx);
		return FFLY_FAILURE;
	}

	if ((this->_3d_uni_pm = (types::_3d_pm_t*)memory::mem_alloc(cnk_c * sizeof(types::_3d_pm_t*))) == NULL) {
		system::io::printf(stderr, "uni_manager: failed to alloc memory for '_3d_uni_pm', errno: %d\n", errno);
		memory::mem_free(this->cnk_id_indx);
	...
	if (this->uni_particles == NULL) {
		system::io::printf(stderr, "uni_manager: failed to alloc memory for 'uni_particles', errno: %d\n", errno);
		memory::mem_free(this->_1d_uni_pm);
	...
	system::io::printf(stdout, "uni_manager: chunk size has been set as %dx%dx%d\n", cnk_xa_len, cnk_ya_len, cnk_za_len);
	static chunk_manager _chunk_manager(cnk_xa_len, cnk_ya_len, cnk_za_len);
	this->_chunk_manager = &_chunk_manager;
	uint_t cnk_point = 0;
	for (types::axis_t za{}; za != this->za_len; za += cnk_za_len) {
		for (types::axis_t ya{}; ya != this->ya_len; ya += cnk_ya_len) {
			for (types::axis_t xa{}; xa != this->xa_len; xa += cnk_xa_len) {
				types::id_t cnk_id = nullptr;

				_chunk_manager.add_cnk(cnk_id, xa, ya, za);

				chunk_data& cnk_data = _chunk_manager.cnk_data(cnk_id);
	...
	system::io::printf(stdout, "uni_manager: there are %d chunk in this uni.\n", cnk_point);
	return FFLY_SUCCESS;
}

mdl::firefly::types::err_t mdl::firefly::uni_manager::draw_cnk(types::off_t __xa_off, types::off_t __ya_off, types::off_t __za_off, types::id_t __cnk_id, types::pixmap_t __pixbuff, uint_t __xa_len, uint_t __ya_len) {
	types::pixmap_t cnk_pixmap;
	if (!(cnk_pixmap = this->_chunk_manager->cnk_data(__cnk_id)._1d_pm)) return FFLY_FAILURE;

	types::err_t any_err;
	if ((any_err = graphics::draw_pixmap(__xa_off, __ya_off, __pixbuff, __xa_len, __ya_len, cnk_pixmap, this->_chunk_manager->get_cnk_xlen(), this->_chunk_manager->get_cnk_ylen())) != FFLY_SUCCESS) return any_err;
	return FFLY_SUCCESS;
}

mdl::firefly::types::_1d_pm_t mdl::firefly::uni_manager::get_1d_cnk_pm(types::axis_t __xa, types::axis_t __ya, types::axis_t __za) {
...
mdl::firefly::types::_3d_pm_t mdl::firefly::uni_manager::get_3d_cnk_pm(types::axis_t __xa, types::axis_t __ya, types::axis_t __za) {
	types::axis_t cnk_xa, cnk_ya, cnk_za;
	this->get_cnk_coords(__xa, __ya, __za, cnk_xa, cnk_ya, cnk_za);
	return this->_3d_cnk_pm(cnk_xa, cnk_ya, cnk_za);
}

void mdl::firefly::uni_manager::get_cnk_coords(types::axis_t __xa, types::axis_t __ya, types::axis_t __za, types::axis_t& __cnk_xa, types::axis_t& __cnk_ya, types::axis_t& __cnk_za) {
...
void mdl::firefly::uni_manager::set_par_colour(types::axis_t __xa, types::axis_t __ya, types::axis_t __za, types::byte_t __r, types::byte_t __g, types::byte_t __b, types::byte_t __a) {
