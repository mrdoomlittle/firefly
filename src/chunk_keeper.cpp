...
boost::int8_t mdl::firefly::chunk_keeper::create_chunk(uint_t*& __chunk_id, bool __overwrite) {
	...
	if (this-> chunk_count == 0) {
		this-> chunk_info = (types::chunk_info_t *)memory::mem_alloc(sizeof(types::chunk_info_t));
		if (this-> chunk_info == NULL) {
			fprintf(stderr, "failed to allocate memory for 'chunk_info', errno: %d\n", errno);
			return FFLY_FAILURE;
		}

		this-> chunk_data = (types::chunk_data_t *)memory::mem_alloc(sizeof(types::chunk_data_t));
		if (this-> chunk_data == NULL) {
			fprintf(stderr, "failed to allocate memory for 'chunk_data', errno: %d\n", errno);
			return FFLY_FAILURE;
		}
	} else {
		this-> chunk_info = (types::chunk_info_t *)realloc(this-> chunk_info, (this-> chunk_count + 1) * sizeof(types::chunk_info_t));
		if (this-> chunk_info == NULL) {
			fprintf(stderr, "failed to relocate memory for 'chunk_info', errno: %d\n", errno);
			return FFLY_FAILURE;
		}

		this-> chunk_data = (types::chunk_data_t *)realloc(this-> chunk_data, (this-> chunk_count + 1) * sizeof(types::chunk_data_t));
		if (this-> chunk_data == NULL) {
	...
	if (!this-> unused_ids.empty()) {
		...
		if (__chunk_id == NULL) {
			fprintf(stderr, "failed to allocate memory for 'chunk_id', errno: %d\n", errno);
			return FFLY_FAILURE;
		}
	...
	if (chunk_data.particles == NULL) {
		fprintf(stderr, "failed to allocate memory for chunk particles, errno: %d\n", errno);
		return FFLY_FAILURE;
	}

	uint_t rxaxis_len = this-> xaxis_len * UNI_PAR_XLEN;
	uint_t ryaxis_len = this-> yaxis_len * UNI_PAR_YLEN;
	uint_t rzaixs_len = this-> zaxis_len * UNI_PAR_ZLEN;
	...
	if (chunk_data.pixmap == NULL) {
		fprintf(stderr, "failed to allocate pixmap for chunk, errno: %d\n", errno);
		return FFLY_FAILURE;
	}
