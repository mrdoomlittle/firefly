# ifndef __tcp__client__hpp
# define __tcp__client__hpp
# include <sys/types.h>
# include <sys/socket.h>
# include <netinet/in.h>
# include <arpa/inet.h>
# include <cstdio>
# include <stdlib.h>
# include <unistd.h>
# include <string.h>
# include <atomic>
# include <mdlint.h>
# include <boost/thread.hpp>
# include <boost/cstdint.hpp>
# include <chrono>
# include <errno.h>
# include "../types/byte_t.h"
# include "../system/errno.h"
# include "../types/err_t.h"
namespace mdl {
namespace firefly {
namespace networking {
class tcp_client
{
	public:
	types::err_t init(char const * __ip_addr, boost::uint16_t __port_no, int& __sock);
	types::err_t de_init();
	types::err_t send(types::byte_t *__buff, uint_t __buff_len);
	types::err_t recv(types::byte_t *__buff, uint_t __buff_len);
	//void poll_handler(int __sock, boost::int8_t *__sock_state, uint_t const __delay_us);
	//boost::int8_t* begin_poll(int __sock, uint_t const __delay_us);

	void poll_handler(uint_t __delay_ns);
	void begin_poll(uint_t __delay_ns);

	static bool to_shutdown;
	std::atomic<uint_t> poll_th_c;
	std::atomic<bool> can_poll;
	std::atomic<bool> poll_lock;
	bool polling_enabled = false;

	template<typename __T>
	types::err_t send(__T __data) {
		std::size_t type_size = sizeof(__T);
		for (std::size_t o = 0; o != type_size; o ++) {
			types::byte_t data_to_send[1] = {0x0};
			if (o == 0) data_to_send[0] = __data & 0xFF;
			else
				data_to_send[0] = (__data >> (o * 8)) & 0xFF;

			types::err_t sock_result = this-> send(data_to_send, 1);
			if (sock_result == FFLY_FAILURE) return FFLY_FAILURE;
		}
		return FFLY_SUCCESS;
	}

	template<typename __T>
	types::err_t recv(__T& __data) {
		std::size_t type_size = sizeof(__T);
		for (std::size_t o = 0; o != type_size; o ++) {
			types::byte_t data_recv[1] = {0x0};

			types::err_t sock_result = this-> recv(data_recv, 1);
			if (sock_result == FFLY_FAILURE) return FFLY_FAILURE;

			__data |= (__data & 0xFF) | (data_recv[0] << (o * 8));
		}
		return FFLY_SUCCESS;
	}

	template<typename __T>
	types::err_t send(__T *__buff, uint_t __buff_len) {
		for (std::size_t o = 0; o != __buff_len; o ++) {
			types::err_t sock_result = this-> send<__T>(__buff[o]);
			if (sock_result == FFLY_FAILURE) return FFLY_FAILURE;
		}
		return FFLY_SUCCESS;
	}

	template<typename __T>
	types::err_t send(__T **__buff, uint_t __buff_xlen, uint_t __buff_ylen) {
		for (std::size_t o = 0; o != __buff_ylen; o ++) {
			types::err_t sock_result = this-> send<__T>(__buff[o], __buff_xlen);
			if (sock_result == FFLY_FAILURE) return FFLY_FAILURE;
		}
		return FFLY_SUCCESS;
	}

	template<typename __T>
	types::err_t recv(__T *__buff, uint_t __buff_len) {
		for (std::size_t o = 0; o != __buff_len; o ++) {
			types::err_t sock_result = this-> recv<__T>(__buff[o]);
			if (sock_result == FFLY_FAILURE) return FFLY_FAILURE;
		}
		return FFLY_SUCCESS;
	}

	template<typename __T>
	boost::int8_t recv(__T *__buff, uint_t __buff_xlen, uint_t __buff_ylen) {
		for (std::size_t o = 0; o != __buff_ylen; o ++) {
			boost::int8_t sock_state = this-> recv<__T>(__buff[o], __buff_xlen);
			if (sock_state == -1) return -1;
		}
		return 0;
	}

	types::err_t connect();

	private:
	int sock;
	struct sockaddr_in serveraddr;
} ;
}
}
}

# endif /*__tcp__client__hpp*/
