...
__kernel void render_camera(__global uint8_t *__uni, __global uint8_t *__camera) {
	//ulong uni_xlen = __uni_xlen[0];
	//ulong uni_coords = ((get_global_id(0) + __offsets[0]) + ((get_global_id(1) + __offsets[1]) * uni_xlen)) * 4;

	//int unsigned cam_coords = (get_global_id(0) + (get_global_id(1) * __uni_xlen)) * 4;
	int unsigned cam_coords = (get_global_id(0) + (get_global_id(1) * get_global_size(0))) * 4;
	__camera[cam_coords] = 255;

	__camera[cam_coords + 3] = 255;
